...
export class FaturaModalComponent implements OnInit {
  ...
  despesas: LancamentoResponseDTO[] = [];
  carregando = true;
  ...
  ngOnInit(): void {
    ...
    if (faturaId) {
      this.reloadTrigger.pipe(
        ...
        switchMap(() => this.lancamentoService.listarPorFatura(faturaId))
      ).subscribe(despesas => {
  ...
  pagarFatura() {
    ...
    if (confirm('Tem certeza que deseja pagar o valor total desta fatura? O valor será debitado da sua conta padrão.')) {
      this.faturaService.pagarFatura(this.data.fatura.id)
  ...
  abrirEdicao(despesa: LancamentoResponseDTO) {
